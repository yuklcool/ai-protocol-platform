...
function renderSection(section: Section, c: RenderContext) {
  ...
  if (section === "skills") return <ResourceChecklist title={t("rootAgent.skills.title")} description={t("rootAgent.skills.description")} empty={t("rootAgent.skills.empty")} items={c.skills.map((skill) => ({ id: skill.skillId, label: skill.displayName || skill.name, detail: skill.description }))} selected={c.selectedSkills} onToggle={(id, checked) => c.toggle("skills", id, checked)} />;
